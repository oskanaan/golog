...
func TestLogSeeker_readFileFromEnd_nonZeroOffset(t *testing.T) {
	...
	actualEntries, actualOffset := readLinesStartingFromPosition(file, 4, 7)
...
func TestLogSeeker_readFileFromEnd_chainNonZeroOffsets(t *testing.T) {
	...
	expectedOffset := 3
	file, _ := os.Open(input)
	_, offset1 := readLinesStartingFromPosition(file, 2, 7)
	actualEntries, actualOffset := readLinesStartingFromPosition(file, 2, offset1)
